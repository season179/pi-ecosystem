...
test("shared quota file: a fresh Pi session sends the fallback first, returns to primary after reset, and sees a global retry", async () => {
  const root = await mkdtemp(join(tmpdir(), "pi-accounts-sdk-"));
  ...
  try {
    ...
    let now = Date.now();
    const quotaPath = join(root, "agent", QUOTA_STATE_FILE);
    const credentials = new InMemoryCredentialStore();
    await credentials.modify("openai-codex", async () => credential("builtin-token"));
    const modelRuntime = await ModelRuntime.create({ credentials, modelsPath: null, refreshOnCreate: false });
    const registry = new ModelRegistry(modelRuntime);
    ...
    let failNext = false;
    let quotaChecks = 0;
    // Each session gets its own extension instance and QuotaStateStore on the same file: a separate Pi process.
    const startSession = async () => {
      ...
      const resourceLoader = new DefaultResourceLoader({
        ...
        noExtensions: true, noSkills: true, noPromptTemplates: true, noThemes: true, noContextFiles: true,
        extensionFactories: [pi => accountsExtension(pi, {
          store, quotaState: new QuotaStateStore(new FileAccountStorageBackend(quotaPath)), providers: [provider], now: () => now,
          checkQuota: async () => { quotaChecks++; return { exhausted: true, resetAt: now + 60_000 }; },
        })],
      });
      await resourceLoader.reload();
      const { session } = await createAgentSession({ cwd: root, agentDir: root, modelRuntime, model,
        sessionManager: SessionManager.inMemory(root), settingsManager, resourceLoader, noTools: "all" });
      await session.bindExtensions({});
      session.agent.streamFunction = (_model, _context, options) => {
        const stream = createAssistantMessageEventStream();
    ...
    failNext = true;
    await first.prompt("first session exhausts the primary");
    assert.deepEqual(requests, ["oc-codex-token", "builtin-token"]);
    assert.equal(first.getLastAssistantText(), "done");
    assert.ok(existsSync(quotaPath));
    assert.ok(!readFileSync(quotaPath, "utf8").includes("token"), "shared file holds labels and times only");

    assert.equal(quotaChecks, 1);
    const second = await startSession();
    await second.prompt("a brand-new session must not retry the known-exhausted primary");
    assert.deepEqual(requests.slice(2), ["builtin-token"], "fallback first, no primary attempt");
    assert.equal(quotaChecks, 1, "no quota check without a failure");
    assert.equal(second.getLastAssistantText(), "done");
    assert.equal(second.sessionManager.getEntries().filter(entry => entry.type === "custom" && entry.customType === "pi-accounts-auto-switch").length, 0);

    now += 61_001;
    await second.prompt("after the real reset the primary is used again");
    assert.deepEqual(requests.slice(3), ["oc-codex-token"]);

    failNext = true;
    await first.prompt("first session exhausts the primary again");
    assert.deepEqual(requests.slice(4), ["oc-codex-token", "builtin-token"]);
    await second.prompt("/accounts-auto retry");
    await first.prompt("retry from another session is visible immediately");
    assert.deepEqual(requests.slice(6), ["oc-codex-token"], "existing controller re-reads shared state; no cache");
    for (const session of sessions) {
      assert.equal(session.messages.filter(message => message.role === "user").length >= 1, true);
      assert.deepEqual(session.messages.filter(message => message.role === "assistant" && message.stopReason === "error"), []);
    }
  } finally {
