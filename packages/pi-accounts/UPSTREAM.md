...
- Codex priority/fallback policy (default `oc-codex` → built-in login when that account exists; persistent `off`), verified usage checks, agent-directory-wide shared cooldowns in `pi-accounts-quota.json`, safe boundary continuation, and `/accounts-auto`.
