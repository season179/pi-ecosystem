...
describe.skipIf(!existsSync(bundlePath))("entry reload mechanism (built bundle required)", () => {
	it("picks up a rebuilt bundle on factory re-invocation and stays pinned otherwise", () => {
		...
		try {
			...
			assert.deepEqual([...verdict.toolsAfterLoad].sort(), ["herdr_orchestrate", "herdr_unwatch", "herdr_watch", "herdr_watches"]);
			assert.deepEqual(verdict.commands.sort(), ["orchestrate", "watches"]);
