...
Warnings are deduplicated per active session by group/state/pressure/reset. Each warning has exactly one surface: a UI notification. Quota text is never persisted into model context: past versions steered a custom message per warning, so the outbound context now scrubs any persisted `pi-herdr-quota-warning` or `pi-herdr-quota-context` messages (exact `customType` matches only; user text with identical wording and `/limits` output are untouched) whether orchestration is active or not. Every LLM call instead receives one transient current snapshot, appended only while orchestration is active and quota groups are configured; appended copies are not written to the session. User-facing `/limits` output is timestamped.
...
- `herdr_route inspect` presents configured candidates, readiness, quota, remaining time, reserve and recent observed burn in a compact per-group/per-candidate summary (full JSON on `verbosity: "full"`). Historical (stale, unavailable or reset-passed) readings are labelled and shown without usable/burn figures; groups sharing one provider/source/account are marked as one subscription. The model judges task needs and pending work; arithmetic is evidence, not an automatic scheduler.
