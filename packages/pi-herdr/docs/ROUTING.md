...
Shares only distinguish equally ranked suitable choices; they cannot override explicit selection, capability, quota exhaustion, or protection. The example can therefore diverge substantially from 40/40/20 when work is mostly easy or hardest. Never generate unnecessary work, wait for a percentage, or use a weaker unsuitable model to balance. Use explicit safe selection when urgency/task-specific judgment differs from configured rank.
