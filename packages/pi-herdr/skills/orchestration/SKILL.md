...
2. **Brief the worker.** Follow the user's model and harness choices and permission requirements. Verify the worker's effective permissions before substantive work; never bypass protections. Brief it with outcome, scope/edit ownership, checkout, constraints, temporary report path, proportional checks, and stopping point. Ask it to write the report and reply with its path. Ensure its environment does not enable `PI_HERDR_ORCHESTRATOR=1`.
3. **Dispatch and watch.** Submit `herdr agent prompt` **without `--wait`**, then arm `herdr_watch`. End your turn unless coordination needs attention. Never block in bash on agent waits or poll transcripts; use bounded command watches for long checks.
